...
import com.ahb.ahbets.service.DepartmentService;
...
/**
 * REST controller for managing {@link com.ahb.ahbets.domain.Department}.
 */
@RestController
@RequestMapping("/api")
...
//        List<DepHosp>  reqHosp = departmentRepository.getBothTbl();
//        System.out.println("PPPPPPPPPPPPPPPPPPPPPP  "+reqHosp.get(0).getDepName()+"   rrrrr");
        if (department.getDeprtId() != null) {
            throw new BadRequestAlertException("A new department cannot already have an ID", ENTITY_NAME, "idexists");
        }
        Department result = departmentService.save(department);
        return ResponseEntity
            .created(new URI("/api/departments/" + result.getDeprtId()))
            .headers(HeaderUtil.createEntityCreationAlert(applicationName, true, ENTITY_NAME, result.getDeprtId().toString()))
            .body(result);
    }
    ...
    public ResponseEntity<Department> updateDepartment(
        ...
        if (!Objects.equals(id, department.getDeprtId())) {
        ...
        return ResponseEntity
            .ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, department.getDeprtId().toString()))
            .body(result);
    }
    ...
    public ResponseEntity<Department> partialUpdateDepartment(
        ...
        return ResponseUtil.wrapOrNotFound(
            result,
            HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, department.getDeprtId().toString())
        );
    }
